	...
	const dataChanged = () => {
		...
		for (let rangeIndex = 0; rangeIndex < videoData.length; rangeIndex++) {
			...
			if (rangeIndex > 0) {
				...
				if (
					(origTransitionData === null && transitionType !== "") ||
					(origTransitionData !== null &&
						(origTransitionData[0] !== transitionType ||
							origTransitionData[1] !== transitionSeconds))
				) {
					return true;
				}
			}
		...
		if (thumbnailType === ThumbnailType.Template) {
			const origThumbnailTemplate =
				props.data?.thumbnail_template ?? initialThumbnailData.template();
		...
		if (
			thumbnailType === ThumbnailType.Frame ||
			thumbnailType === ThumbnailType.Template ||
			thumbnailType === ThumbnailType.CustomTemplate
		) {
			const origThumbnailTime =
				dateTimeFromWubloaderTime(props.data?.thumbnail_time ?? null) ??
				initialThumbnailData.time();
			...
			if (
				thumbnailTime !== null &&
				origThumbnailTime !== null &&
				!thumbnailTime.equals(origThumbnailTime)
			) {
				return true;
			}
		}
		if (
			...
			if (!cropMatches) {
				...
				cropMatches =
					origThumbnailCrop[0] === thumbnailCrop[0] &&
					origThumbnailCrop[1] === thumbnailCrop[1] &&
					origThumbnailCrop[2] === thumbnailCrop[2] &&
					origThumbnailCrop[3] === thumbnailCrop[3];
			}
			if (!cropMatches) {
				return true;
			}
			const origThumbnailLocation =
				props.data?.thumbnail_location ?? initialThumbnailData.location();
			...
			if (!locationMatches) {
				...
				locationMatches =
					origThumbnailLocation[0] === thumbnailLocation[0] &&
					origThumbnailLocation[1] === thumbnailLocation[1] &&
					origThumbnailLocation[2] === thumbnailLocation[2] &&
					origThumbnailLocation[3] === thumbnailLocation[3];
			}
			if (!locationMatches) {
				return true;
			}

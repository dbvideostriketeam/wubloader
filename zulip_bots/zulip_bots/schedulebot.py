...
def update_groups(client, group_ids, schedule, hour, start_time, last):
	if hour < 0:
...
def check_reminders(reminders: list[Reminder], hour, start_time, shifts, send_client):
	if hour < 0: return # Don't send reminders before the run starts
	moonbase_start = datetime.fromtimestamp(start_time, timezone(shifts['timezone']))
...
def main(conf_file, hour=-1, no_groups=False, stream="General", no_mentions=False, no_initial=False, shifts=None, last=-1, metrics_port=8012, reminder: list[str] = None):
	...
	if hour >= 0:
		...
		if not no_groups:
			update_groups(client, group_ids, schedule, hour, start_time, last)
		if stream:
	...
	while True:
		...
		if not no_initial:
			...
			if stream:
				post_schedule(client, send_client, start_time, schedule, stream, hour, no_mentions, last, shifts)
		no_initial = False
		check_reminders(reminders, hour, start_time, shifts, send_client)
